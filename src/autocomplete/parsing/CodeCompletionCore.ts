...
import { Parser, ParserRuleContext, Token, TokenStream, Vocabulary } from 'antlr4ts';
...
const longestCommonPrefix = <T>(arr1: T[] | undefined, arr2: T[] | undefined): T[] => {
  if (!arr1 || !arr2) {
    return [];
  }
...
export class CodeCompletionCore {
        ...
        if (label && label.size > 0) {
          if (transition.serializationType === TransitionType.NOT_SET) {
            label = label.complement(
              IntervalSet.of(Token.MIN_USER_TOKEN_TYPE, this.atn.maxTokenType)
            );
          }
          const set = new FollowSetWithPath();
      ...
      if (this.preferredRules.has(startState.ruleIndex)) {
        ...
        if (followSets.sets) {
          for (const set of followSets.sets) {
            ...
            if (!this.translateStackToRuleIndex(fullPath) && set.intervals) {
                    ...
                    if (!this.ignoredTokens.has(symbol)) {
                      ...
                      if (!this.candidates.tokens.has(symbol)) {
                        ...
                        this.candidates.tokens.set(
                          symbol,
                          longestCommonPrefix(followingTokens, this.candidates.tokens.get(symbol))
                        );
                      }
                    }
                  }
  ...
  private generateBaseDescription(state: ATNState): string {
    const stateValue =
      state.stateNumber === ATNState.INVALID_STATE_NUMBER ? 'Invalid' : state.stateNumber;
    const typeName = CodeCompletionCore.atnStateTypeMap[state.stateType];
  ...
  private printDescription(
    ...
    if (this.debugOutputWithTransitions) {
      for (const transition of state.getTransitions()) {
        ...
        const typeName = CodeCompletionCore.atnStateTypeMap[transition.target.stateType];
