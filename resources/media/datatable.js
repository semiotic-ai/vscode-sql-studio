						...
						(function () {
							...
							function m(t, e, s) {
								...
								switch (o) {
									...
									case s._const.addElement:
										...
										i.insertBefore(
											p(e[s._const.element], 'http://www.w3.org/2000/svg' === i.namespaceURI, s),
											i.childNodes[c] || null
										);
										break;
							...
							var w = function (t) {
									...
									return (
										...
										'#text' !== t.nodeName &&
											...
											(t.attributes.class &&
												e.push(
													''.concat(t.nodeName, '.').concat(t.attributes.class.replace(/ /g, '.'))
												),
											t.attributes.id &&
												...
												var t = (function (t, e, s, i) {
													...
													for (var h = 0; h < o; h++)
														for (var u = t[h], p = 0; p < r; p++) {
															...
															s[h] || i[p] || !M(u, f, d, c)
																? (l[h + 1][p + 1] = 0)
																: ((l[h + 1][p + 1] = l[h][p] ? l[h][p] + 1 : 1),
																	l[h + 1][p + 1] >= n &&
																		((n = l[h + 1][p + 1]), (a = [h + 1, p + 1])));
														}
								...
								return (
									e.forEach(function (e) {
										!(function (t, e, s) {
											var i, n, a, o;
											if (
												![s._const.addElement, s._const.addTextElement].includes(e[s._const.action])
											) {
								...
								R = function (t, e) {
									...
									return (
										t.replace(k, function (r, l) {
											...
											if (d) {
												if (
													('svg' === (s = P(r, e.caseSensitive || o)).node.nodeName && (o = !0),
													n++,
													!s.voidElement && u && '<' !== u)
												) {
											...
											if (
												...
												(n > -1 &&
													(s.voidElement ||
														(e.caseSensitive && s.node.nodeName === r.slice(2, -1)) ||
														(!e.caseSensitive &&
															s.node.nodeName.toUpperCase() === r.slice(2, -1).toUpperCase())) &&
													--n > -1 &&
												...
												0 === (s = this.findNextDiff(t, e, [])).length &&
													(D(t, e) ||
														(this.foundAll
															...
															: ((this.foundAll = !0), N(t), (s = this.findNextDiff(t, e, []))))),
													s.length > 0 &&
														((this.foundAll = !1), this.tracker.add(s), C(t, s, this.options));
											} while (s.length > 0);
														...
														if ('#text' === (l = f[w]).nodeName)
															if ('#text' === m[v].nodeName) {
																if (l.data !== m[v].data) {
																	for (
																		var _ = w;
																		f.length > _ + 1 && '#text' === f[_ + 1].nodeName;
								...
								q = (
										...
										childNodes: [
											{
												...
												childNodes: s.map(({ row: t, index: e }) => {
														...
														childNodes: t.cells
															.map((t, s) => {
																...
																const r = {
																	...
																	childNodes:
																		'html' === o.type ? t.data : [{ nodeName: '#text', data: n(t) }]
																};
																if (
																	(d ||
																		h ||
																	...
																	o.cellClass &&
																		(r.attributes.class = c(r.attributes.class, o.cellClass)),
																	o.render)
																) {
													...
													if (
														(e === o && (s.attributes.class = c(s.attributes.class, r.cursor)), g)
													) {
														const i = g(t, s, e);
														if (i) {
															if ('string' != typeof i) return i;
									...
									if (((x.attributes.class = c(x.attributes.class, r.table)), d || h || _)) {
										...
										if (d || _) {
											...
											(!f.length && !l) || w || (e.attributes = { style: 'height: 0px;' }),
															...
															f = function (t, e) {
																return M.w(
																	s
																		...
																		[
																			t
																		].apply(s.toDate('s'), (i ? [0, 0, 0, 0] : [23, 59, 59, 999]).slice(e)),
																	s
																);
															},
															m = this.$W,
							...
							const W = (t, e) => {
								...
								Z = (t, e = void 0, s, n, a) => {
									...
									if (t.headings) o.headings = t.headings.map((t) => X(t));
												...
												return (
													(s =
														...
														(1 === e.childNodes.length && '#text' === e.childNodes[0].nodeName)
															...
															: { data: e.childNodes, type: 'html', text: i(e) }),
										...
										e?.tBodies.length &&
											(o.headings = Array.from(e.tBodies[0].rows[0].cells).map((t) => X('')));
							...
							class G {
								...
								setCursor(t = !1) {
									...
									const e = this.cursor;
									if (
										((this.cursor = t), this.dt._renderTable(), !1 !== t && this.dt.options.scrollY)
									) {
							...
							class nt {
									...
									if (this.modalDOM) {
										...
										if ('Enter' === t.key) {
											if (this.editingCell) {
												...
												const t = Array.from(
													this.dt.wrapperDOM.querySelectorAll(`input${e}[type=text]`)
												).map((t) => t.value.trim());
												this.saveRow(t, this.data.row);
											}
										} else
								...
								saveCell(t) {
									const e = this.data.content,
										s =
											this.dt.columns.settings[this.data.columnIndex].type || this.dt.options.type,
										i = t.trim();
									let n;
								...
								editRowModal() {
									...
									t.forEach((t, e) => {
										...
										if (
											...
											const i =
												this.dt.data.headings[e].text || String(this.dt.data.headings[e].data);
											r.insertBefore(
												s('div', {
									...
									if (t) {
										...
										i.cells = e.map((e, i) => {
											...
											if ('number' === n) o = { data: parseFloat(a) };
												...
												o = { data: [{ nodeName: '#text', data: a }], text: a, order: a };
							...
							(exports.DataTable = class {
								...
								_renderSeparateHeader() {
									...
									if (
										((e.attributes.class = c(e.attributes.class, this.options.classes.table)),
									...
									this.options.searchable &&
												...
												(Array.from(this.wrapperDOM.querySelectorAll(e))
													...
													.forEach((t) => {
														...
														e
															? s.forEach((e) => {
																	t.dataset.columns
																		? i.push({ terms: [e], columns: JSON.parse(t.dataset.columns) })
										...
										this.options.rowNavigation
											? (this.dom.addEventListener('keydown', (t) => {
												...
												this.dom.addEventListener('mousedown', (t) => {
													...
													if (e instanceof Element && this.dom.matches(':focus')) {
														const s = Array.from(this.dom.querySelectorAll('tbody > tr')).find(
															(t) => t.contains(e)
														);
														s &&
											...
											: this.dom.addEventListener('mousedown', (t) => {
													const e = t.target;
													if (!(e instanceof Element)) return;
													const s = Array.from(this.dom.querySelectorAll('tbody > tr')).find((t) =>
														t.contains(e)
													);
													s &&
														s instanceof HTMLElement &&
								...
								multiSearch(t) {
									...
									this.data.data.forEach((t, e) => {
										...
										s.every((t) =>
											t.find((t, e) => !!t && t.find((t) => i[e].find((e) => e.includes(t))))
								...
								setMessage(t) {
									const e =
										this.data.headings.filter((t, e) => !this.columns.settings[e]?.hidden).length ||
										1;
									this.options.classes.empty
										?.split(' ')
