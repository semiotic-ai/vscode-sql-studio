...
function_call
    : identifier_nontype LEFT_PAREN (set_qualifier? vex_or_named_notation (COMMA vex_or_named_notation)* orderby_clause?)? RIGHT_PAREN
...
select_primary
    ...
    | TABLE ONLY? identifier MULTIPLY?
    | values_stmt
    ;
...
from_primary
    : ONLY? identifier MULTIPLY? alias_clause? (TABLESAMPLE method=identifier LEFT_PAREN vex (COMMA vex)* RIGHT_PAREN (REPEATABLE vex)?)?
